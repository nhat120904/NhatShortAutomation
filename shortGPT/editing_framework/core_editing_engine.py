...
class CoreEditingEngine:
    ...
    def generate_image(self, schema: Dict[str, Any], output_file, logger=None):
        assets = dict(
            sorted(schema["visual_assets"].items(), key=lambda item: item[1]["z"])
        )
        clips = []
    ...
    def generate_video(
        ...
        audio_assets = dict(
            sorted(schema["audio_assets"].items(), key=lambda item: item[1]["z"])
        )
        ...
        for asset_key in visual_assets:
            ...
            if asset_type == "video":
                ...
                try:
                    ...
                    print(
                        f"Failed to load image {asset['parameters']['url']}. Error : {str(e)}"
                    )
                    continue
            elif asset_type == "text":
                clip = self.process_text_asset(asset)
    ...
    ) -> Union[VideoFileClip, AudioFileClip, ImageClip, TextClip]:
    ...
    def process_common_visual_actions(
        self, clip: Clip, actions: List[Dict[str, Any]]
    ) -> Union[VideoFileClip, ImageClip, TextClip]:
        ...
        for action in actions:
            ...
            if action["type"] == "green_screen":
                ...
                clip = clip.with_effects(
                    [vfx.MaskColor(color=color, threshold=thr, stiffness=s)]
                )
                continue
    ...
    def process_audio_actions(
        self, clip: AudioClip, actions: List[Dict[str, Any]]
    ) -> AudioClip:
        clip = self.process_common_actions(clip, actions)
        for action in actions:
    ...
    def process_video_asset(self, asset: Dict[str, Any]) -> VideoFileClip:
        ...
        if params["filename"].startswith(("http://", "https://")):
            ...
            original_filename = url.split("/")[-1].split("?")[
                0
            ]  # Remove query parameters
            if len(original_filename) > 50:  # Limit filename length
                file_ext = (
                    os.path.splitext(original_filename)[1] or ".mp4"
    ...
    def process_image_asset(self, asset: Dict[str, Any]) -> ImageClip:
        ...
        if url.startswith(("http://", "https://")):
            ...
            if len(original_filename) > 50:  # Limit filename length
                file_ext = (
                    os.path.splitext(image_filename)[1] or ".jpg"
    ...
    def process_text_asset(self, asset: Dict[str, Any]) -> TextClip:
        ...
        if not (any(key in text_clip_params for key in ["text", "fontsize", "size"])):
            raise Exception(
                "You must include at least a size or a fontsize to determine the size of your text"
            )
    ...
    def _convert_m3u8_to_mp4(self, m3u8_path):
        ...
        try:
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                ...
                raise Exception(
                    "Conversion completed but output file is missing or empty"
                )
