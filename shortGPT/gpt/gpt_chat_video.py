...
def generateScript(script_description, language):
    ...
    chat, system = gpt_utils.load_local_yaml_prompt(
        "prompt_templates/chat_video_script.yaml"
    )
    chat = chat.replace("<<DESCRIPTION>>", script_description).replace(
        "<<LANGUAGE>>", language
    )
...
def correctScript(script, correction):
    out = {"script": ""}
    chat, system = gpt_utils.load_local_yaml_prompt(
        "prompt_templates/chat_video_edit_script.yaml"
    )
    chat = chat.replace("<<ORIGINAL_SCRIPT>>", script).replace(
