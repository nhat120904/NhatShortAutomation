...
class AbstractContentEngine(ABC):
    ...
    def prepareEditingPaths(self):
        self.dynamicAssetDir = (
            f".editing_assets/{self.dataManager.contentType}_assets/{self.id}/"
        )
    ...
    def makeContent(self):
        while not self.isShortDone():
            ...
            if self.stepDict[currentStep].__name__ == "_editAndRenderShort":
                ...
                yield currentStep, f"Current step ({currentStep} / {self.get_total_steps()}) : " + self.stepDict[
                    currentStep
                ].__name__
    ...
    def initializeFFMPEG(self):
        ...
        if not ffmpeg_path:
            raise Exception(
                "FFmpeg, a program used for automated editing within ShortGPT was not found on your computer. Please go back to the README and follow the instructions to install FFMPEG"
            )
        ffprobe_path = get_program_path("ffprobe")
        if not ffprobe_path:
            raise Exception(
                "FFProbe, a dependecy of FFmpeg was not found. Please go back to the README and follow the instructions to install FFMPEG"
            )
