...
from shortGPT.audio.audio_utils import (
    audioToText,
    get_asset_duration,
...
from shortGPT.editing_utils.captions import getCaptionsWithTime, getSpeechBlocks
...
class MultiLanguageTranslationEngine(AbstractContentEngine):
    ...
    def _transcribe_audio(self):
        ...
        if not (
            cached_translation
            and "speech_blocks" in cached_translation
            and "original_language" in cached_translation
        ):
        ...
        if ACRONYM_LANGUAGE_MAPPING.get(self._db_original_language) == Language(
            self._db_target_language
        ):
    ...
    def _translate_content(self):
        if self._db_should_translate:
            ...
            for i, ((t1, t2), text) in tqdm(
                enumerate(self._db_speech_blocks), desc="Translating content"
            ):
                self.logger(
                    f"2/5 - Translating text content - {i+1} / {len(self._db_speech_blocks)}"
                )
    ...
    def _generate_translated_audio(self):
        self.verifyParameters(
            translated_timed_sentences=self._db_translated_timed_sentences
        )

        translated_audio_blocks = []
        for i, ((t1, t2), translated_text) in tqdm(
            enumerate(self._db_translated_timed_sentences),
            desc="Generating translated audio",
        ):
            self.logger(
                f"3/5 - Generating translated audio - {i+1} / {len(self._db_translated_timed_sentences)}"
            )
            translated_voice = self.voiceModule.generate_voice(
                translated_text,
                self.dynamicAssetDir + f"translated_{i}_{self._db_target_language}.wav",
            )
            ...
            final_audio_path = speedUpAudio(
                ...
                + f"translated_{i}_{self._db_target_language}_spedup.wav",
                expected_duration=t2 - t1 - 0.05,
            )
            _, translated_duration = get_asset_duration(final_audio_path, isVideo=False)
            translated_audio_blocks.append(
                [[t1, t1 + translated_duration], final_audio_path]
            )
    ...
    def _edit_and_render_video(self):
        ...
        editing_engine.addEditingStep(
            ...
            {"url": input_video, "set_time_start": 0, "set_time_end": video_length},
        )
        last_t2 = 0
        for (t1, t2), audio_path in self._db_audio_bits:
            ...
            editing_engine.addEditingStep(
                ...
                {"url": audio_path, "set_time_start": t1, "set_time_end": t2},
            )
            if t1 - last_t2 > 4:
                editing_engine.addEditingStep(
                    ...
                    {
                        "url": video_audio,
                        "subclip": {"start_time": last_t2, "end_time": t1},
                        "set_time_start": last_t2,
                        "set_time_end": t1,
                    },
                )
            last_t2 = t2
        ...
        if video_length - last_t2 > 4:
            editing_engine.addEditingStep(
                ...
                {
                    "url": video_audio,
                    "subclip": {"start_time": last_t2, "end_time": video_length},
                    "set_time_start": last_t2,
                    "set_time_end": video_length,
                },
            )
        ...
        if self._db_use_captions:
            ...
            if not self._db_timed_translated_captions:
                if not self._db_translated_voiceover_path:
                    self.logger(
                        f"4.5 / 5 - Generating captions in {target_language.value}"
                    )
                    editing_engine.generateAudio(
                        self.dynamicAssetDir + "translated_voiceover.wav"
                    )
                    self._db_translated_voiceover_path = (
                        self.dynamicAssetDir + "translated_voiceover.wav"
                    )
                whispered_translated = audioToText(
                    self._db_translated_voiceover_path, model_size="base"
                )
                timed_translated_captions = getCaptionsWithTime(
                    whispered_translated,
                    maxCaptionSize=50 if is_landscape else 15,
                    considerPunctuation=True,
                )
                self._db_timed_translated_captions = [
                    [[t1, t2], text]
                    for (t1, t2), text in timed_translated_captions
                    if t2 - t1 <= 4
                ]
