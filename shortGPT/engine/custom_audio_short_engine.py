...
class CustomAudioShortEngine(ContentShortEngine):
    ...
    def __init__(
        ...
        super().__init__(
            short_id=short_id,
            short_type="custom_audio_shorts",
    ...
    def _prepareBackgroundAssets(self):
        ...
        if self._use_background_image:
            ...
            if not self._db_background_trimmed:
                self.logger(
                    "Rendering short: (2/4) preparing background video asset..."
                )
    ...
    def _generateTempAudio(self):
        ...
        if not self._db_custom_audio_path:
            raise ValueError(
                "Custom audio path cannot be empty. Please upload an audio file."
            )
        ...
        try:
            if self._db_custom_audio_path.lower().endswith(
