        ...
        with gr.Row(visible=False) as short_automation:
            with gr.Column():
                ...
                short_type = gr.Radio(
                    ...
                    label="Type of shorts generated",
                    value="Reddit Story shorts",
                    interactive=True,
                )
                facts_subject = gr.Textbox(
                    label="Write a subject for your facts (example: Football facts)",
                    interactive=True,
                    visible=False,
                )
                custom_text = gr.Textbox(
                    label="Write your custom text content for the video",
                    interactive=True,
                    visible=False,
                    lines=5,
                ...
                custom_audio = gr.File(
                    label="Upload your audio file (MP3, WAV, M4A, etc.)",
                    interactive=True,
                    visible=False,
                    file_types=["audio"],
                )
                display_text = gr.Textbox(
                    label="Text to display in the video",
                    interactive=True,
                    visible=False,
                    lines=5,
                    placeholder="Enter the text you want to display in the video...",
                )
                ...
                tts_info = gr.HTML(
                    "ℹ️ TTS settings are not used for Custom Audio shorts or Text Display shorts since you're either uploading your own audio or displaying text only",
                    visible=False,
                )
                ...
                def update_tts_visibility(short_type_val):
                    ...
                    return (
                        gr.update(
                            visible=not (is_custom_audio or is_text_display)
                        ),  # tts_engine
                        gr.update(
                            visible=(is_custom_audio or is_text_display)
                        ),  # tts_info
                        gr.update(
                            visible=not (is_custom_audio or is_text_display)
                            and self.tts_engine == AssetComponentsUtils.ELEVEN_TTS
                        ),  # eleven_tts
                        gr.update(
                            visible=not (is_custom_audio or is_text_display)
                            and self.tts_engine == AssetComponentsUtils.EDGE_TTS
                        ),  # edge_tts
                    )
                ...
                short_type.change(
                    ...
                    [short_type],
                    [tts_engine, tts_info, eleven_tts, edge_tts],
                )

                def tts_engine_change(x):
                    self.tts_engine = x
                    return gr.update(
                        visible=x == AssetComponentsUtils.ELEVEN_TTS
                ...
                numImages = gr.Radio(
                    [5, 10, 25],
                    value=10,
                    label="Number of images per short",
                    visible=True,
                    interactive=True,
                )
                ...
                watermark = gr.Textbox(
                    label="Watermark (your channel name)", visible=False
                )
                addWatermark.change(
                    lambda x: gr.update(visible=x), [addWatermark], [watermark]
                )
                ...
                background_type.change(
                    ...
                    [background_type],
                    [background_video_section, background_image_section],
                )
            ...
            video_folder.click(
                lambda _: AssetComponentsUtils.start_file(os.path.abspath("videos/"))
            )

            createButton.click(
                self.inspect_create_inputs,
                inputs=[
                    AssetComponentsUtils.background_video_checkbox(),
                    AssetComponentsUtils.background_music_checkbox(),
                    AssetComponentsUtils.background_image_checkbox(),
    ...
    def create_short(
        ...
        try:
            ...
            if background_type == "Background Video":
                background_videos = (
                    background_video_list
                    * ((numShorts // len(background_video_list)) + 1)
                )[:numShorts]
                ...
                background_images = (
                    background_image_list
                    * ((numShorts // len(background_image_list)) + 1)
                )[:numShorts]
            ...
            if background_music_list:
                background_musics = (
                    background_music_list
                    * ((numShorts // len(background_music_list)) + 1)
                )[:numShorts]
            else:
                background_musics = [
                    ""
                ] * numShorts  # Empty string for no background music
            ...
            if short_type in ["Custom Audio shorts", "Text Display shorts"]:
                ...
                language = (
                    Language.ENGLISH
                )  # Default language, not used for audio processing
            ...
            for i in range(numShorts):
                ...
                yield self.embedHTML + "</div>", gr.update(visible=True), gr.update(
                    visible=False
                )
    ...
    def inspect_create_inputs(
        ...
        if short_type == "Custom Audio shorts":
            ...
            if custom_audio and hasattr(custom_audio, "name"):
                audio_ext = (
                    custom_audio.name.lower().split(".")[-1]
                    if "." in custom_audio.name
                    else ""
                )
                allowed_exts = ["mp3", "wav", "m4a", "aac", "flac", "ogg"]
                if audio_ext not in allowed_exts:
                    raise gr.Error(
                        f"Unsupported audio format. Please use: {', '.join(allowed_exts)}"
                    )
        ...
        if short_type not in [
            ...
            if not openai_key and not gemini_key:
                raise gr.Error(
                    "GEMINI OR OPENAI API key is missing. Please go to the config tab and enter the API key."
                )
        ...
        if short_type not in ["Custom Audio shorts", "Text Display shorts"]:
            ...
            if (
                self.tts_engine == AssetComponentsUtils.ELEVEN_TTS
                and not eleven_labs_key
            ):
                raise gr.Error(
                    "ELEVENLABS_API_KEY API key is missing. Please go to the config tab and enter the API key."
                )
        return gr.update(visible=False)

    def create_short_engine(
        self,
        short_type,
        voice_module,
        language,
        ...
        if short_type == "Reddit Story shorts":
            return RedditShortEngine(
                ...
                background_music_name=background_music or "",
        ...
        if short_type == "Custom Text shorts":
            return CustomTextShortEngine(
                voice_module,
                custom_text=custom_text,
                background_video_name=background_video or "",
                background_music_name=background_music or "",
                background_image_name=background_image or "",
                num_images=numImages,
                watermark=watermark,
                language=language,
                video_effect=video_effect,
