...
class VideoTranslationUI(AbstractComponentUI):
    def __init__(self, shortGptUI: gr.Blocks):
        ...
        self.eleven_language_choices = [
            lang.value.upper() for lang in ELEVEN_SUPPORTED_LANGUAGES
        ]
    ...
    def create_ui(self):
        with gr.Row(visible=False) as video_translation_ui:
            with gr.Column():
                videoType = gr.Radio(
                    ["Youtube link", "Video file"],
                    label="Input your video",
                    value="Youtube link",
                    interactive=True,
                )
                video_path = gr.Video(
                ...
                yt_link = gr.Textbox(
                    label="Youtube link (https://youtube.com/xyz): ",
                    interactive=True,
                    visible=False,
                )
                videoType.change(
                    lambda x: (
                        gr.update(visible=x == "Video file"),
                        gr.update(visible=x == "Youtube link"),
                    ),
                    [videoType],
                    [video_path, yt_link],
                )
                ...
                with gr.Column(visible=False) as eleven_tts:
                    language_eleven = gr.CheckboxGroup(
                        self.eleven_language_choices,
                        label="Language",
                        value="ENGLISH",
                        interactive=True,
                    )
                    voice_eleven = AssetComponentsUtils.voiceChoiceTranslation(
                        provider=AssetComponentsUtils.ELEVEN_TTS
                    )
                with gr.Column(visible=True) as edge_tts:
                    language_edge = gr.CheckboxGroup(
    ...
    def translate_video(
        ...
        if tts_engine == AssetComponentsUtils.ELEVEN_TTS:
            languages = [
                Language(lang.lower().capitalize()) for lang in language_eleven
            ]
        ...
        try:
            for i, language in enumerate(languages):
                ...
                content_translation_engine = MultiLanguageTranslationEngine(
                    ...
                    src_url=yt_link if videoType == "Youtube link" else video_path,
                ...
                def logger(prog_str):
                    progress(
                        self.progress_counter / (num_steps),
                        f"Translating your video ({i+1}/{len(languages)}) - {prog_str}",
                    )
                ...
                for step_num, step_info in content_translation_engine.makeContent():
                    progress(
                        self.progress_counter / (num_steps),
                        f"Translating your video ({i+1}/{len(languages)}) - {step_info}",
                    )
                ...
                yield "<div>" + self.embedHTML + "</div>", gr.update(
                    visible=True
    ...
    def inspect_create_inputs(
        ...
        supported_extensions = [
            ...
            ".avi",
            ".mov",
        ]  # Add more supported video extensions if needed
        print(videoType, video_path, yt_link)
        if videoType == "Youtube link":
            if not yt_link.startswith(
                "https://youtube.com/"
            ) and not yt_link.startswith("https://www.youtube.com/"):
                raise gr.Error(
                    "Invalid YouTube URL. Please provide a valid URL. Link example: https://www.youtube.com/watch?v=dQw4w9WgXcQ"
                )
        else:
            ...
            if file_ext not in supported_extensions:
                raise gr.Error(
                    "Invalid video file. Supported video file extensions are: {}".format(
...
def update_progress(progress, progress_counter, num_steps, num_shorts, stop_event):
    ...
    while not stop_event.is_set():
        ...
        progress(
            progress_counter / (num_steps * num_shorts),
            f"Rendering progress - {dynamic}/3649",
        )
