...
class AssetLibrary(AbstractComponentUI):
    ...
    def create_ui(self):
        ...
        with gr.Tab("Asset library") as asset_library_ui:
            with gr.Column():
                with gr.Accordion(
                    "➕ Add your own local assets or from Youtube", open=False
                ) as accordion:
                    ...
                    with gr.Column(visible=True) as youtubeFlow:
                        ...
                        asset_type = gr.Radio(
                            [
                                ...
                                AssetType.BACKGROUND_MUSIC.value,
                            ],
                            value=AssetType.BACKGROUND_VIDEO.value,
                            label="Type",
                        )
                    ...
                    with gr.Column(visible=False) as localFileFlow:
                        ...
                        video_upload = gr.Video(
                            visible=True, sources="upload", interactive=True
                        )
                        ...
                        upload_type.change(
                            lambda x: (
                                gr.update(visible="video" in x),
                                gr.update(
                                    visible=any(
                                        type in x for type in ["audio", "music"]
                                    )
                                ),
                                gr.update(visible=x == "image"),
                            ),
                            [upload_type],
                            [video_upload, audio_upload, image_upload],
                        )
                    assetFlows.change(
                        lambda x: (
                            gr.update(visible=x == remote),
                            gr.update(visible=x == local),
                        ),
                        [assetFlows],
                        [youtubeFlow, localFileFlow],
                    )
                with gr.Row():
                    with gr.Column(scale=3):
                        asset_dataframe_ui = gr.Dataframe(
                            self.__fulfill_df, interactive=False
                        )
                        video_choise = gr.Radio(
                    ...
                    with gr.Column(scale=2):
                        ...
                        delete_button = gr.Button(
                            "🗑️ Delete", scale=0, variant="primary"
                        )
                        delete_button.click(
                            self.__delete_clicked,
                            [delete_button],
                            [
                                asset_dataframe_ui,
                                asset_preview_ui,
                        ...
                        asset_dataframe_ui.select(
                            self.__preview_asset,
                            [asset_dataframe_ui],
                            [asset_preview_ui, delete_button],
                        )

                add_youtube_link.click(
                    self.__verify_youtube_asset_inputs,
                    [asset_name, youtube_url, asset_type],
                    [],
                ).success(
                    self.__add_youtube_asset,
                    [asset_name, youtube_url, asset_type],
                    [
                        asset_dataframe_ui,
                        asset_preview_ui,
                    ...
                    lambda: gr.update(open=False), [accordion]
                )

                upload_button.click(
                    self.__verify_and_upload_local_asset,
                    [
                        upload_type,
                        local_upload_name,
                        video_upload,
    ...
    def __verify_youtube_asset_inputs(self, asset_name, yt_url, type):
        if not asset_name or not re.match("^[A-Za-z0-9 _-]*$", asset_name):
            raise gr.Error(
                "Invalid asset name. Please provide a valid name that you will recognize (Only use letters and numbers)"
            )
        if not yt_url.startswith("https://youtube.com/") and not yt_url.startswith(
            "https://www.youtube.com/"
        ):
            raise gr.Error("Invalid YouTube URL. Please provide a valid URL.")
        if AssetDatabase.asset_exists(asset_name):
            raise gr.Error(
                "An asset already exists with this name, please choose a different name."
            )
    ...
    def __add_youtube_asset(self, asset_name, yt_url, type):
        ...
        return (
            gr.update(value=latest_df),
            gr.update(value=self.__get_asset_embed(latest_df, 0)),
            ...
            gr.update(
                choices=AssetComponentsUtils.getBackgroundMusicChoices(),
                interactive=True,
            ),
            gr.update(
                choices=AssetComponentsUtils.getBackgroundImageChoices(),
                interactive=True,
            ),
        )
    ...
    def __preview_asset(self, data, evt: gr.SelectData):
        ...
        return gr.update(value=html_embed), gr.update(
            value=f"🗑️ Delete {data.iloc[evt.index[0]]['name']}"
        )
    ...
    def __verify_and_upload_local_asset(
        self, upload_type, upload_name, video_path, audio_path, image_path
    ):
        ...
        return self.__upload_local_asset(
            upload_type, upload_name, video_path, audio_path, image_path
        )

    def __upload_local_asset(
        self, upload_type, upload_name, video_path, audio_path, image_path
    ):
        ...
        new_path = (
            "public/"
            + self.__clean_filename(upload_name)
            + "."
