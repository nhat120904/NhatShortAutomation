...
def test_video_effect_options():
    ...
    try:
        from shortGPT.editing_utils.video_effects import get_video_effect_options, get_effect_parameters, VideoEffect
        ...
        for name, effect in options.items():
            ...
            print(f"  - {name}: {effect.value} {list(params.keys()) if params else '(no params)'}")
...
def test_editing_step_enum():
    ...
    try:
        ...
        assert hasattr(EditingStep, 'APPLY_VIDEO_EFFECT'), "APPLY_VIDEO_EFFECT should be in EditingStep enum"
        print("✓ APPLY_VIDEO_EFFECT editing step found")
        
        # Check the value
        assert EditingStep.APPLY_VIDEO_EFFECT.value == "apply_video_effect.json", "Incorrect JSON file name"
